            ...
            <div id="idTab1" class="product-info">  
                ...
                <form id="buy_block" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0}class=""{/if} action="{$link->getPageLink('cart.php')}" method="post">
                    ...
                    <ul>
                                    ...
                                    {if $product->show_price AND !isset($restricted_country_mode) AND !$PS_CATALOG_MODE}
                                    	<li>                                    	
                                            <p class="price">
                                                ...
                                                {elseif 0 AND $product->specificPrice AND $product->specificPrice.reduction AND $productPriceWithoutRedution >$productPrice}
                                                    <span class="discount">{l s='Reduced price!'}</span>
                                                {/if}                                                
                                                <span class="our_price_display" {if $product->specificPrice AND $product->specificPrice.reduction AND $productPriceWithoutRedution >$productPrice}style="color:#DA0F00;"{/if}>
                                                {if $priceDisplay >= 0 && $priceDisplay <= 2}
                                                    <span id="our_price_display">{convertPrice price=$productPrice}</span>
                                                    {if 0 && $tax_enabled  && ((isset($display_tax_label) && $display_tax_label == 1) OR !isset($display_tax_label))}
                                                        ...
                                                        {if $productPriceWithoutRedution > $productPrice}
                                                            <span id="old_price_display">{convertPrice price=$productPriceWithoutRedution}</span>
								                                            ...
								                                            {if $product->specificPrice AND $product->specificPrice.reduction_type == 'percentage'}
								                                                <span id="reduction_percent">{l s='Bạn tiết kiệm: '} <span id="reduction_percent_display">{$product->specificPrice.reduction*100}</span> %</span>
																														{elseif $product->specificPrice AND $product->specificPrice.reduction_type == 'amount'}
						                                                		<span id="reduction_amount">{l s='Bạn tiết kiệm: '} <span id="reduction_amount_display">{math equation="x/y*z" x=$product->specificPrice.reduction y=$productPriceWithoutRedution z=100 format="%.0f"}</span> %</span>
								                                            {/if}

                                                            {if 0 && $tax_enabled && $display_tax_label == 1}
                                                                {if $priceDisplay == 1}{l s='tax excl.'}{else}{l s='tax incl.'}{/if}
                                                            {/if}
                                                        {/if}
                                                    {/if}
                            ...
                            {if $product->description_short OR $packItems|@count > 0}
                                <div class="pro-description">   
																...
																{if $features}
																	...
																	<div><span style="position:relative;top:-5px;">Cách giặt ủi </span>
																	{foreach from=$features item=feature name="washfeats"}

																			{if file_exists("{$smarty.const._PS_ROOT_DIR_}/images/features/black/{$feature.id_feature_value}.png")}<img height="24" width="24" src="/images/features/black/{$feature.id_feature_value}.png" />{else}{if $wash_i}</div><div><span>Chất liệu:</span> {/if}{assign var="wash_i" value="0"}{$feature.value|escape:'htmlall':'UTF-8'} <span>{$feature.name|replace:'_':''|escape:'htmlall':'UTF-8'}</span>{if $smarty.foreach.washfeats.last}{else}, {/if}{/if}
                        ...
                        <span class="first-item">Màu sắc:</span>
                                    <div id="color_picker">
                            ...
                            <span class="last-item">
                            	...
                            	<p id="add_to_cart">
                            	    <input type="image" src="{$smarty.const._THEME_DIR_}images/btn/add-to-bag.gif" class="btn-add-cart"/>
																	<span style="display:block; margin-top:4px; color: #007236;">Giao hàng trong ngày!</span>
                               </p>
