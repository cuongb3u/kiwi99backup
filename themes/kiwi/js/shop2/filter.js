...
$(document).ready(function() {
	$('.tree-menu-selected').parents('.top_menu_help').find('.top_menu_help_a').addClass('tree-menu-selected');
...
filterQuery = 'filter=1&id_attribute=' + strAttribute + '&hasColor=' + hasColor + '&hasPrice=' + hasPrice + '&brand=' + strBrand + '&prices=' + strPrice + '&orderby=' + orderby + '&orderway=' + orderway + '&n=' + itemperpage + addtionQuery + '&is_search=' + is_search + '&search_query=' + search_query;
		...
		$.ajax({
			type: 'GET',
			url: baseDir + 'category.php?shop=2&id_category=' + id_category + '&' + $('#left-cat-tree a:first').attr('href').split('?',2)[1],
			async: true,
			cache: false,
			dataType: "json",
			data: filterQuery,
...
filterQuery = 'filter=1&id_attribute=' + strAttribute + '&hasColor=' + hasColor + '&hasPrice=' + hasPrice + '&brand=' + strBrand + '&prices=' + strPrice + '&orderby=' + orderby + '&orderway=' + orderway + '&n=' + itemperpage;
	...
	// $('a.product_img_link').live('mouseover', function(){
	// 	
	// 		current_src = $('img', this).attr('src');
	// 		
	// 		console.log(current_src);
	// 		
	// 		$('#color-images img').each(function(index){
	// 			
	// 			console.log($(this).parent().attr('href'));
	// 			if($(this).parent().attr('href') == current_src){
	// 				console.log('hfhashfhsa');
	...
	$('body#category .paginationclass a').live('click', function() {
		...
		updatedQuery = $(this).attr('href');

		if(parseInt(updatedQuery.search('http')) == -1 ){
			updatedQuery = updatedQuery.slice(1);
			updatedQuery = updatedQuery.split('&').slice(2).join('&');
		}
		...
		if (pt == -1) {
			...
			if(is_search && search_query && parseInt(updatedQuery.search('search_query')) == -1){
					updatedQuery += '&is_search=' + is_search + '&search_query=' + search_query;
			}
...
//			alert('search');
	...
	$('body#category #left-cat-tree a').click(function() {
		...
		*/
		...
		// if(!$('.data #ajax-loading').length){
		// 	$('#product-list #ajax-loading').clone().appendTo('.data');
		// 	$('#product-list #ajax-loading').remove();			
		// }
	...
	link2=$(this).attr('href');	
		top_menu_select=$('body#category .main-menu a[href='+link2+']');
		$(top_menu_select).addClass('tree-menu-selected');
		$(top_menu_select).parents('.top_menu_help').find('.top_menu_help_a').addClass('tree-menu-selected')
		...
		$.ajax({
			type: 'GET',
			url: baseDir + 'category.php?shop=2&id_category=' + id_cat + '&' + $('#left-cat-tree a:first').attr('href').split('?',2)[1],
			async: true,
			cache: false,
			dataType: "json",
			data: filterQuery,
			...
			complete: function() {
					...
					mmax = parseInt($('#product-filter').attr('max'));
					sliderval = mmin + ';' + mmax;
					repstr = '	<div class="layout-slider" style="position:absolute; margin: 0 270px; width:320px;"><input id="Slider2" type="slider" name="price" value="'+sliderval+'" /></div>';
			...
			success: function(jsonData, textStatus, jqXHR) {
				...
				$('.data .box-adv').html(jsonData.catimgtpl);
				$('.paginationclass').replaceWith(jsonData.pagination);
				$('#productList').html(jsonData.productList);
		...
		// $('.data #ajax-loading').clone().appendTo('#product-list');
		// $('.data #ajax-loading').remove();
	...
	$('ul.main-menu li h3 a').click(function() {
		...
		*/
		...
		$(this).addClass('tree-menu-selected');
		$(this).parents('.top_menu_help').find('.top_menu_help_a').addClass('tree-menu-selected');
		id_cat = $(this).attr('id_cat');
