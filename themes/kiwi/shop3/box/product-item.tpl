...
{if isset($product.combinationImages) && $product.combinationImages}
	...
	<img width="16" height="16" alt="{$product.name|escape:'htmlall':'UTF-8'}" src="{$img_col_dir}{$color_info.id_attribute}.png" id="thumb_{$color_info.id_attribute}">
	</a>
	</li>
			{/foreach}
...
{/if}
    ...
    <h4><a href="{$product.link|escape:'htmlall':'UTF-8'}" title="{$product.name|escape:'htmlall':'UTF-8'}">{$product.name|truncate:35:'...'|escape:'htmlall':'UTF-8'}</a></h4>
    <h5>                            	            	
    ...
    <h6>
        <a href="#">{if isset($product.available_for_order) && $product.available_for_order && !isset($restricted_country_mode)}{if ($product.allow_oosp || $product.quantity > 0)}{l s='Available'}{elseif (isset($product.quantity_all_versions) && $product.quantity_all_versions > 0)}{l s='Product available with different options'}{else}{l s='Out of stock'}{/if}{/if}</a>
    </h6>    
</div>