...
class CustomerCore extends ObjectModel
{
	...
	protected $tables = array ('customer');
 	...
 	protected 	$fieldsSize = array( 'passwd' => 32, 'firstname' => 32, 'email' => 128, 'note' => 65000);
	...
	public static function getCustomers()
	{
		...
		SELECT `id_customer`, `email`, `firstname`
	...
	public static function getNewsletteremails()
	{
		Tools::displayAsDeprecated();
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
		SELECT `email`, `firstname`, `newsletter`, `ip_registration_newsletter`, `newsletter_date_add`
		FROM `'._DB_PREFIX_.'customer`
		WHERE `newsletter` = 1
		AND `active` = 1');
	}
	...
	*/
	...
	public function transformToCustomer($id_lang, $password = NULL)
	{
		...
		if ($this->update())
		{
			...
			Mail::Send((int)$id_lang, 'guest_to_customer', Mail::l('Your guest account has been transformed to customer account'), $vars, $this->email, $this->firstname);
			return true;
		}
		return false;
	}
