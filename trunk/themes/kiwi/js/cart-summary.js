...
$(document).ready(function()
{
	...
	if (window.ajaxCart !== undefined)
	{
		...
		$('.cart_quantity_delete' ).unbind('click').click(function(){ 
			//$(this).parents('.cart_item').fadeOut('slow').remove();
			deletProductFromSummary($(this).attr('id')); return false; });
       ...
       success: function(jsonData)
       {
    		...
    		else
    		{
    			if (parseInt(jsonData.summary.products.length) == 0)
    			{
					...
					$('.item ').fadeOut('slow').html('<p class="warning warning1">Your shopping cart is empty.</p>').fadeIn();
