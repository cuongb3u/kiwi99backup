...
$(document).ready(function() {
	...
	var firstTimeClickVoucher = true;
	
	
	
			
	$('form#voucher').submit(function(){
		
//		console.log(this);
		
		var actionTarget = $(this).attr('action');
		
		var voucher = $('#discount_name').val();
		...
		$.ajax({
		  ...
		  complete: function(xhr, textStatus) {
		    //called when complete
				if(firstTimeClickVoucher){
					
					firstTimeClickVoucher = false;
										
					$('form#voucher').submit();				

				}else{
				
					firstTimeClickVoucher = true;
		  ...
		  success: function(data, textStatus, xhr) {
				...
				if(data.errors.length){
					if(firstTimeClickVoucher){
						...
						$('#voucherErrors').addClass('error');
						$('#voucherErrors').html(data.errors[0]);
						
					}
//					firstTimeClickVoucher = false;
				}else{
				
					$('#voucherErrors').removeClass('error');
					$('#voucherErrors').html('OK');
					
//					firstTimeClickVoucher = false;
				}

				$('#enteredVouchers').html(data.shopping_cart);
				$('#step1 .shipping').html(data.shippingClass);
...
function updateAddreslist()
{
	...
	$.ajax({
	  ...
	  type: "GET",
	  dataType: "html",
	  data: 'ajax_list_address=1',
	
	  complete: function() {
	    updateAddressesDisplay();
	    $('div..box-address').ajaxloading('remove');
	  },
	
	  success: function(html) {
	   $('#step1 .box-address').html(html);
	 },
...
function updateAddressSelection()
{
	...
	$.ajax({
           ...
           dataType : "json",
           data: 'ajax=true&method=updateAddressesSelected&id_address_delivery=' + $('#id_address_delivery').val() + '&id_address_invoice=' + $('#id_address_delivery').val() + '&token=' + static_token,
	...
	$.ajax({
		...
		success: function(json)
		{
			...
			$('#opc_new_account').fadeOut('fast', function() {
				$('#opc_new_account').replaceWith(json.order_opc_adress);
...
$(function() {
	...
	if ((!isLogged) || (isGuest))
	{
		...
		$('#submitAccount').click(function() {
			if ($('#email').data('check')==0) {
				jAlert('Vui lòng chọn email khác', 'Thông báo');
			...
			if (!$("#new_account_form").validationEngine('validate')) {
	...
	$('.btn_add_address').live('click',function() {
		...
		$.ajax({
		  ...
		  success: function(html) {
		      ...
		      content='<div id="add_address">'+html+'</div>';
		      $('.btn_add_address').showContent('add',content);
		 },
	...
	$('div.dim_content').live('click',function()
		{
