...
<p>
	<img src="{$smarty.const._THEME_DIR_}images/icon/pdf.gif" alt="" class="icon" />
...
{/if}
{if $order->recyclable}
<p><img src="{$smarty.const._THEME_DIR_}images/icon/recyclable.gif" alt="" class="icon" />&nbsp;{l s='You have given permission to receive your order in recycled packaging.'}</p>
{/if}
{if $order->gift}
	<p><img src="{$smarty.const._THEME_DIR_}images/icon/gift.gif" alt="" class="icon" />&nbsp;{l s='You requested gift-wrapping for your order.'}</p>
	<p>{l s='Message:'} {$order->gift_message|nl2br}</p>
{/if}
<div id="orderDetail" style="clear:both;width:100%;float:left;">
    <ul class="address">
        <li class="address_title">{l s='Invoice'}</li>
        {foreach from=$inv_adr_fields name=inv_loop item=field_item} 
        	{if $field_item != 'company' && $field_item != 'vat_number' && $field_item != 'address2'}       
                {if $field_item eq "address2" && $address_invoice->address2}<li class="address_address2">{$address_invoice->address2|escape:'htmlall':'UTF-8'}</li>
                ...
                {else}
                        ...
                        <li><span class="first-item">{if $field_item eq 'firstname lastname'}Họ tên{elseif $field_item eq 'address1'}Địa chỉ{elseif $field_item eq 'postcode city'}Tỉnh/thành{elseif $field_item eq 'Country:name'}Quốc gia{else}Điện thoại{/if}:</span><span class="last-item">{foreach from=$address_words item=word_item name="word_loop"}{if !$smarty.foreach.word_loop.first} {/if}<span class="address_{$word_item}">{if $word_item == 'city'}{$invoiceAddressFormatedValues[$word_item]|escape:'htmlall':'UTF-8'|provincename}{else}{$invoiceAddressFormatedValues[$word_item]|escape:'htmlall':'UTF-8'}{/if}</span>{/foreach}</span></li>
                {/if}
        	{/if}
        {/foreach}
    </ul>
...
<div id="order-detail-content" class="table_block">
	<table class="std" cellpadding="0" cellspacing="0" width="100%" border="0">
		<thead>
			<tr>
		...
		<tfoot>
			...
			{/if}
			<tr class="item">
				<td colspan="{if $return_allowed}6{else}5{/if}" align="right">
...
{else}
<p><img src="{$smarty.const._THEME_DIR_}images/icon/infos.gif" alt="" class="icon" />&nbsp;{l s='You can\'t make a merchandise return with a guest account'}</p>
{/if}
