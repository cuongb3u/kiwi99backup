                        ...
                        *}
                            ...
                            <p class="text">
                                ...
                                {if isset($customerThread.email)}
                                    <input type="text" id="email" class="validate[required,custom[email]]" name="from" value="{if isset($customerThread.email)}{$customerThread.email}{/if}" readonly="readonly" />
                                {else}
                                    <input type="text" id="email" class="validate[custom[email]]" name="from" value="{if isset($email)}{$email}{/if}" />
                                {/if}
                            </p>
                        {if !$PS_CATALOG_MODE}
                                ...
                                {if !isset($customerThread.id_product)}
                                    <select id="select_product" name="id_product" width="500">
                                    	...
                                    	{foreach from=$products_order item=product name=products_order}
                                    	 ...
                                    	 <option value="{$product.product_id}" block="{$count_block++}" id_order="{$product.id_order}" >{$product.name}</option>
                                    {/foreach}
                                    </select>
                                    <div id="product_help">
                                    	{$count_block=0}
                                    	{foreach from=$products_order item=product name=products_order}
                                    	<div class="block_product" block_product="{$count_block++}" reference_product="{$product.reference}">
                                    	...
                                    	<div class="product_help_attribute">
                                    		...
                                    		{/foreach}
                                    		 {if isset($product.id_manufacturer)}
                          					  <li class="product_brand" > <img src="../img/m/{$product.id_manufacturer}.jpg"  border="0"/></li>
                       		 		   {/if}
                                    		</ul>
                                    	</div>
