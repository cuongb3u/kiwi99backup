...
<div id="cart_block" class="block exclusive">    
	<div class="block_content">
		{*
    	<div class="adv-item">
            <img src="{$smarty.const._THEME_DIR_}images/data/adv.5.gif" alt="" border="0"/>
        </div>
        *}
    	<div class="block_item">
        	<h4>   
                ...
                <span class="ajax_cart_product_txt_s" {if $cart_qties <= 1}style="display:none"{/if}>{l s='sản phẩm' mod='blockcart'}</span>
                <span class="ajax_cart_product_txt" {if $cart_qties > 1}style="display:none"{/if}>{l s='sản phẩm' mod='blockcart'}</span>
                <p class="ajax_cart_total" {if $cart_qties <= 0}style="display:none"{/if}>{if $priceDisplay == 1}{convertPrice price=$cart->getOrderTotal(false)}{else}{convertPrice price=$cart->getOrderTotal(true)}{/if}</p>
                <span class="ajax_cart_no_product" {if $cart_qties != 0}style="display:none"{/if}>{l s='(trống)' mod='blockcart'}</span>
            </div>  *}
               {* <a href="{$link->getPageLink("$order_process.php", true)}">{l s='Giỏ hàng' mod='blockcart'}</a>*}
                {if $ajax_allowed}
            ...
            <div id="cart_block_list" class="{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)}expanded{else}collapsed{/if}">    
                        ...
                        {foreach from=$products item='product' name='myLoop'}
                            ...
                            <dt id="cart_block_product_{$product.id_product}{if $product.id_product_attribute}_{$product.id_product_attribute}{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
                                ...
                                <a class="cart_block_product_name" href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category)}" title="{$product.name|escape:html:'UTF-8'}">
                                	  <img src= "{$link->getImageLink($product.link_rewrite, $product.pai_id_image, 'medium')}" alt="{$product.name|truncate:13:'...'|escape:html:'UTF-8'}" class="product_cart_image"/>
                        		<ul class="product_cart_attr">
                        			 {if isset($product.id_manufacturer)}
                          		
                       		    {/if}
                                <li class="product_cart_name">{$product.name|truncate:22:'...'|escape:html:'UTF-8'}</li>
                              <li class="product_cart_quantity">Số lượng : <span class="quantity">{$product.cart_quantity}</span</li>
                              {if isset($product.attributes)}
                              {assign var="attributes" value=","|explode:$product.attributes}
                              {foreach from=$attributes item=attribute}

                             <li title="{l s='Product attributes'}">{$attribute}</li>
                              {/foreach}
                              {/if}
                ...
                {/if}                               
            	<p {if $products}class="hidden"{/if} id="cart_block_no_products">{l s='Không có sản phẩm nào' mod='blockcart'}</p>          
                <p id="cart-prices">
                ...
                <p id="cart-buttons">
                        ...
                        <a id="close-minibasket" href="javascript:closeUI();"><img src="{$smarty.const._THEME_DIR_}images/icon/close.gif" alt="" border="0"/> <span>Đóng cửa sổ</span></a>
                    ...
                    <a href="{$link->getPageLink("$order_process.php", true)}{if $order_process == 'order'}?step=1{/if}" id="button_order_cart" class="exclusive{if $order_process == 'order-opc'}_large{/if}" title="{l s='Check out' mod='blockcart'}">
                        <img src="{$smarty.const._THEME_DIR_}images/btn/check-out.gif" alt="" border="0"/>
                    </a>
                </p>
           	</div>
           	 <div class="minicartslot">
			<p style="padding: 0px 10px;">Order today and we deliver within 4-6 working days</p>
		</div>
        </div>
