...
class Blocknewsletter extends Module
{
 	...
 	private function newsletterRegistration()
 	{
		...
		//		return $this->error; 
	 	...
	 	elseif ($_POST['action'] == '1')
	 	{
	 	 	...
	 	 	if ($registerStatus < 1)
	 	 		return $this->error = $this->l('E-mail đã được đăng ký!');
	 	 	...
	 	 	elseif ($registerStatus == 1)
	 	 	{
			  	...
			  	echo $this->error = $this->l('Error during unsubscription');
			  	echo $this->valid = $this->l('Unsubscription successful');
				die;
	 	...
	 	elseif ($_POST['action'] == '0')
	 	{
			...
			if ($registerStatus > 0)
				{
					$this->error = $this->l('E-mail đã được đăng ký!');
				echo $this->error;
				die;
			}
			/* If the user ins't a customer */
			elseif ($registerStatus == -1)
			{
				...
				echo $this->valid = $this->l('Đăng ký thành công!');
				die;
			}
			/* If the user is a customer */
	...
	function hookCommonFooter($params)
		{
			return $this->hookLeftColumn($params);
		}
