...
class BlockCategories extends Module
{
	...
	public function getTree($resultParents, $resultIds, $maxDepth, $id_category = 1, $currentDepth = 0)
	{
		...
		if (!isset($resultIds[$id_category]))
			return false;
		return array('id' => $id_category, 'link' => $link->getCategoryLink($id_category, $resultIds[$id_category]['link_rewrite']),'link_rewrite'=>$resultIds[$id_category]['link_rewrite'],
	...
	public function hookLeftColumn($params)
	{
		...
		if (!$this->isCached('blockcategories.tpl', $smartyCacheId))
		{
			...
			foreach ($result as &$row)
			{
			 ...
			 if ($shop) {
			 	$shop_category = explode('_',_SHOP_CATEGORY_);
			 

			 	$blockCategTree = $this->getTree($resultParents, $resultIds, Configuration::get('BLOCK_CATEG_MAX_DEPTH'),$id_category = (int)$shop_category[$shop]);
			 	if ($shop==3) {
			 			//echo json_encode($blockCategTree);die;
			 	}
